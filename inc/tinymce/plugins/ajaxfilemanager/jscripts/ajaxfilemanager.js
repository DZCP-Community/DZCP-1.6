...
function appendQueryString(baseUrl, queryStr, excludedQueryStr) {
    ...
    if (typeof(excludedQueryStr) == 'object' && excludedQueryStr.length) {
        ...
        baseUrl = urlParts[0];
        var count = 1;
        if (typeof(urlParts[1]) != 'undefined' && urlParts[1] != '') {//this is the query string parts
    ...
    if (view != '') {
        ...
        switch (view) {
            ...
            case 'thumbnail':
                ...
                for (i in files) {
                    if (files[i].type == 'folder') {//this is foder item

                        enableFolderBrowsable(i);
                    } else {//this is file item
...
function ajaxStart(destinationSelector, id, selectorOfAnimation) {
    if (typeof(selectorOfAnimation) == 'undefined') {//set defaullt animation
...
function deleteDocuments() {
    ...
    if (isSelected) {//remove them via ajax call
        var options =
            {
                dataType: 'json',
                url: getUrl('delete'),
...
function addDocumentHtml(num) {
    ...
    switch (getView()) {
        ...
        case 'thumbnail':
            ...
            if (files[num].type == 'folder') {//this is foder item
