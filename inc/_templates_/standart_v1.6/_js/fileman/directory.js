...
function Directory(fullPath, numDirs, numFiles, name) {
    ...
    this.name = name ? name : RoxyUtils.GetFilename(fullPath);
    ...
    this.GetHtml = function () {
        var html = '<li data-path="' + this.fullPath + '" data-dirs="' + this.dirs + '" data-files="' + this.files + '" data-name="' + this.name + '" class="directory">';
        html += '<div><img src="images/fileman/' + (this.dirs > 0 ? 'dir-plus.png' : 'blank.gif') + '" class="dirPlus" width="9" height="9">';
        html += '<img src="images/fileman/folder.png" class="dir"><span class="name">' + this.name + ' (' + this.files + ')</span></div>';
        html += '</li>';
    ...
    this.Select = function (selectedFile) {
        ...
        el.children('img.dir').prop('src', 'images/fileman/folder.png');
        this.SetStatusBar();
    ...
    this.SetOpened = function () {
        ...
        if (li.find('li').length < 1)
            li.children('div').children('.dirPlus').prop('src', 'images/fileman/blank.gif');
        else if (this.IsExpanded())
            li.children('div').children('.dirPlus').prop('src', 'images/fileman/dir-minus.png');
        else
            li.children('div').children('.dirPlus').prop('src', 'images/fileman/dir-plus.png');
    };
        ...
        $.ajax({
            ...
            success: function (dirs) {
                ...
                for (i = 0; i < dirs.length; i++) {
                    var d = new Directory(dirs[i].p, dirs[i].d, dirs[i].f, dirs[i].n);
                    d.Show();
                }
                $('#pnlLoadingDirs').hide();
                $('#pnlDirList').show();
