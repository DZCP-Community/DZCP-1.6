...
( function() {
    ...
    // Maintain the map of smiley-to-description. (DZCP 1.7)
    var smileyReverseMap = {}, smileyRegExp = [], smileyMap = {};
    $.ajax({
        url: "../../../../inc/ajax.php?i=bbcode&get=smileys",
        async: true,
        dataType: 'json',
        success: function(data) {
            smileyMap = data.smiley_map;
            ...
            //Smileys
            CKEDITOR.config.smiley_images = data.smiley_images;
            CKEDITOR.config.smiley_descriptions = data.smiley_descriptions;
            CKEDITOR.config.smiley_columns = data.smiley_columns;
            CKEDITOR.config.smiley_path = data.smiley_path;
                ...
                elements: {
                    $: function( element ) {
                        ...
                        if ( tagName in convertMap )
                            ...
                            var responsive = (attributes.alt);
                            var width = 0, height = 0;

                            if(!responsive && attributes.width) { width = attributes.width; } //Get width
                            if(!responsive && attributes.height) { height = attributes.height; } //Get height

                            var allowfullscreen = (attributes.hasOwnProperty('allowfullscreen'));

                            var control_parameter = getParameterByName("controls",attributes.src);
                            var controls = (control_parameter !== null && control_parameter == 0);

                            var rel_parameter = getParameterByName("rel",attributes.src);
                            var rel = (rel_parameter !== null && rel_parameter >= 1);

                            var autoplay_parameter = getParameterByName("autoplay",attributes.src);
                            var autoplay = (autoplay_parameter !== null && autoplay_parameter >= 1);

                            var time = false;
                            var time_parameter = getParameterByName("start",attributes.src);
                            if(time_parameter !== null) {
                                time = time_parameter;
                            }

                            var nocookie = (attributes.src.toLocaleLowerCase().indexOf("youtube-nocookie") != -1);

                            //Extract ID
                            var res = attributes.src.split("embed/");
                            res = res[1].split("?");
                            var vid = res[0];
                            ...
                            var test = "[youtube"+obj_width+obj_height+obj_autoplay+obj_allowfullscreen+
                                    obj_nocookie+obj_rel+obj_controls+obj_responsive+obj_time+"]"+vid+"[/youtube]";

                            console.log(test);

                            tagName = 'youtube';
                            element.children = [ new CKEDITOR.htmlParser.text( test ) ];
                        }
