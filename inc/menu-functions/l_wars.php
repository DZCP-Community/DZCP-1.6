    ...
    if (_rows($qry)) {
        while ($get = _fetch($qry)) {
            ...
            if (config('allowhover') == 1 || config('allowhover') == 2)
                $info = 'onmouseover="DZCP.showInfo(\'' . up(re($get['name'])) . ' vs. ' . up(re($get['gegner'])) . '\', \'' . _played_at . ';' . _cw_xonx . ';' . _result . ';' . _comments_head . '\', \'' . date("d.m.Y H:i", $get['datum']) . _uhr . ';' . up(re($get['xonx'])) . ';' . cw_result_nopic_nocolor($get['punkte'], $get['gpunkte']) . ';' . cnt($db['cw_comments'], "WHERE cw = '" . $get['id'] . "'") . '\')" onmouseout="DZCP.hideInfo()"';
