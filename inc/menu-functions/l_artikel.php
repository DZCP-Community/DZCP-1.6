    ...
    if (_rows($qry)) {
        while ($get = _fetch($qry)) {
            ...
            if (config('allowhover') == 1)
                $info = 'onmouseover="DZCP.showInfo(\'' . up(re($get['titel'])) . '\', \'' . _datum . ';' . _autor . ';' . _news_admin_kat . ';' . _comments_head . '\', \'' . date("d.m.Y H:i", $get['datum']) . _uhr . ';' . fabo_autor($get['autor']) . ';' . up(re($getkat['kategorie'])) . ';' . cnt($db['acomments'], "WHERE artikel = '" . $get['id'] . "'") . '\')" onmouseout="DZCP.hideInfo()"';
