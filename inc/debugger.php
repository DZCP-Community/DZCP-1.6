...
class DebugConsole {
     ...
     */
    public static final function logToFile($file='debug_text',$content="") {
        if(file_exists(basePath.'/inc/_logs_/'.$file.'.log'))
            $content = file_get_contents(basePath.'/inc/_logs_/'.$file.'.log').$content;

        file_put_contents(basePath.'/inc/_logs_/'.$file.'.log', $content);
    }
    ...
    public static final function show_logs() {
        ...
        </style><table bgcolor="#000000" style="margin-top: 56px;" width="100%" border="0" ><tr><td bgcolor="#00FF00"><span class="boxdebug" style="font-size:11px">Debug Log: ( '.$i.' Eintr&auml;ge )<a name="log" id="log"></a></span></td>
