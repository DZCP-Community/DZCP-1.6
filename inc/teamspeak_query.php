...
function teamspeakViewer() {
  return (settings('ts_version') == 3) ? teamspeak3() : teamspeak2();
}
...
function teamspeak2() {
      @set_time_limit(10);
        $fp = @fsockopen(settings('ts_ip'), settings('ts_sport'), $errno, $errstr, 2);
        ...
        if($fp)
      {
        ...
        $uArr = getTSChannelUsers(settings('ts_ip'),settings('ts_port'),settings('ts_sport'));
        $pcArr = Array();
        $ccArr = Array();
        $thisArr = Array();
        $listArr = Array();
        $usedArr = Array();
        $cArr    = getChannels(settings('ts_ip'),settings('ts_port'),settings('ts_sport'));
        $z = 0;
        $x = 0;
