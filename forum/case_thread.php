                ...
                if ($toCheck) {
                    ...
                    $qry = db("INSERT INTO " . $db['f_threads'] . "
                                 SET     `kid`      = '" . ((int)$_GET['kid']) . "',
                                                ...
                                                `subtopic` = '" . up($_POST['subtopic']) . "',
                                                `t_nick`   = '" . $chkMe && $userid ? data('nick') : up($_POST['nick']) . "',
                                                `t_email`  = '" . $chkMe && $userid ? data('email') : up($_POST['email']) . "',
                                                `t_hp`     = '" . $chkMe && $userid ? data('hp') : up(links(re($_POST['hp'], true))) . "',
                                                `t_reg`    = '" . ((int)$userid) . "',
                                                `t_text`   = '" . up($_POST['eintrag']) . "',
                                                `sticky`   = '" . isset($_POST['sticky']) ? ((int)$_POST['sticky']) : 0 . "',
                                                `global`   = '" . isset($_POST['global']) ? ((int)$_POST['global']) : 0 . "',
                                                `ip`       = '" . $userip . "',
                                                `lp`       = '" . time() . "',
                                                `vote`     = '" . $vid . "',
                                                `first`    = '1'");
