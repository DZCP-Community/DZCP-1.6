...
switch ($do) {
    case 'default_server':
        $qry = db("SELECT `id` FROM `".$db['teamspeak']."` WHERE `default_server` = 1;");
        foreach($qry as $get) {
            db("UPDATE `".$db['teamspeak']."` SET `default_server` = 0 WHERE `id` = ".$get['id'].";");
        }

        db("UPDATE `".$db['teamspeak']."` SET `default_server` = 1 WHERE `id` = ".intval($_GET['id']).";");
        header("Location: ?admin=teamspeak");
    break;
    case 'menu':
        $ts3_menu = db("SELECT `id` FROM `".$db['teamspeak']."` WHERE `show_navi` = 1;",false,true);
        $qry = db("SELECT `id` FROM `".$db['teamspeak']."` WHERE `show_navi` = 1;");
        foreach($qry as $get) {
            db("UPDATE `".$db['teamspeak']."` SET `show_navi` = 0 WHERE `id` = ".intval($get['id']).";");
        }
        
        if($ts3_menu['id'] != intval($_GET['id'])) {
            db("UPDATE `".$db['teamspeak']."` SET `show_navi` = 1 WHERE `id` = ".intval($_GET['id']).";");
        }

        header("Location: ?admin=teamspeak");
    break;
    case 'delete':
        $get = $sql->fetch("SELECT `host_ip_dns`,`server_port` FROM `".$db['teamspeak']."` WHERE `id` = ? LIMIT 1;",array(intval($_GET['id'])));
        ...
        $sql->delete("DELETE FROM `".$db['teamspeak']."` WHERE `id` = ?;",array(intval($_GET['id'])));
    ...
    case 'edit':
            ...
            if(empty($error)) {
                if(isset($_POST['defaults'])) {
                    ...
                    while($get = _fetch($qry)) {
                        db("UPDATE `".$db['teamspeak']."` SET `default_server` = 0 WHERE `id` = ?;",array($get['id'])); 
                    }
                }

                db("UPDATE `".$db['teamspeak']."` SET `host_ip_dns` = ?,`server_port` = ?,"
        ...
        if(empty($show)) {
            $get = db("SELECT * FROM `".$db['teamspeak']."` WHERE `id` = ".intval($_GET['id']).";",false,true);
            $show = show($dir."/teamspeak_edit", array('id' => intval($_GET['id']),
                                                       ...
                                                       'button_value_edit' => _button_value_edit,
    ...
    case 'new':
        ...
        if(isset($_POST['ip'])) {
            ...
            if(empty($error)) {
                ...
                db("INSERT INTO `".$db['teamspeak']."` SET `host_ip_dns` = '".up($_POST['ip'])."',`server_port` = ".
                    intval($_POST['port']).",`query_port` = ".intval($_POST['sport']).","
                    . "`customicon` = ".intval($_POST['customicon']).",`showchannel` = ".
                    intval($_POST['showchannel']).",`default_server` = ".(isset($_POST['defaults']) ? 1 : 0).",`show_navi` = 0;");
        ...
        if(empty($show))
            $show = show($dir."/teamspeak_add", array('error' => (!empty($error) ? show("errors/errortable", array("error" => $error)) : ""),
                                                      ...
                                                      'button_value_add' => _button_value_add,
    ...
    break;
    default:
        $qry = db("SELECT * FROM `".$db['teamspeak']."` ORDER BY id;"); $color = 1;
        while($get = _fetch($qry)) {
        ...
        $show = show($dir."/teamspeak", array("show" => $show,"admin_server_new"=>_admin_server_new));
    break;
}
