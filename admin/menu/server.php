...
switch ($do) {
    ...
    case 'editserver':
        if(empty($_POST['ip']) || empty($_POST['port'])) {
            ...
            $cache->deleteItem('nav_server_'.intval($_GET['id']));

            db("UPDATE `".$db['server']."` SET `ip` = '".up($_POST['ip']).
                "', `port` = ".(int)($_POST['port']).
                ", `qport` = ".(int)$_POST['qport'].
                ", `name` = '".up($_POST['name']).
                "', `custom_icon`= '".up($_POST['custom_game_icon']).
                "', `icon` = '', `game` = '".($_POST['status'] != 'lazy' ? up($_POST['status']) : $get['game']).
                "', `pwd` = '".up($_POST['pwd'])."' WHERE `id` = ".intval($_GET['id']).";");
    ...
    case 'new':
        ...
        $show = show($dir."/server_add", array("games" => listgame($games,false), "button_value_add" => _button_value_add, "custom_icon" => $custom_icon));
    break;
    case 'add':
        if(empty($_POST['ip']) || empty($_POST['port'])) {
            $show = error(_empty_ip);
    ...
    default:
        ...
        while($get = _fetch($qry)) {
            ...
            if(!$icon && !empty($get['icon'])) {
                $game_icon_inp = search_game_icon(re($get['icon']));
        ...
        $show = show($dir."/server", array("show" => $show_servers, "admin_server_new" => _admin_server_new));
    break;
}
