        ...
        while($getcustom = _fetch($qrycustom)) {
            ...
            if(!empty($getcontent[$getcustom['feldname']])) {
                switch($getcustom['type']) {
                    case 2:
                        $custom_content .= show(_profil_custom_url, array("name" => re($getcustom['name']), "value" => re($getcontent[$getcustom['feldname']])));
                        break;
                    case 3:
                        $custom_content .= show(_profil_custom_mail, array("name" => re($getcustom['name']), "value" => eMailAddr(re($getcontent[$getcustom['feldname']]))));
                        break;
                    default:
                        $custom_content .= show(_profil_custom, array("name" => re($getcustom['name']), "value" => re($getcontent[$getcustom['feldname']])));
                        break;
                }
            }
