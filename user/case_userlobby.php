...
if (defined('_UserMenu')) {
    ...
    if ($chkMe) {
        ...
        $qrycw = db("SELECT s1.*,s2.`icon` FROM `" . $db['cw'] . "` AS `s1` LEFT JOIN `" . $db['squads'] . "` AS `s2` ON s1.`squad_id` = s2.`id` ORDER BY s1.`datum`;");
        $cws = '';
        if (_rows($qrycw) >= 1) {
        ...
        if (!empty($getgb) && check_new((int)$getgb['datum'])) {
            $cntgb = "";
            if (!$permission_gb && settings('gb_activ'))
                $cntgb = "AND `public` = 1";
        ...
        $nextkal = '';
        if (!empty($getkal) && check_new((int)$getkal['datum'])) {
        ...
        if (_rows($qryawayn) >= 1) {
            ...
            while ($getawayn = _fetch($qryawayn)) {
                if (check_new((int)$getawayn['date']) && data('level') >= 2) {
