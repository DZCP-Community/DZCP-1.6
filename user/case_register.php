...
if(defined('_UserMenu')) {
    ...
    if(!$chkMe) {
        ...
        if($check_regcode) {
            $regcode = show($dir."/register_regcode", array("confirm" => _register_confirm, "confirm_add" => _register_confirm_add,));
        }
    ...
    if ($do == "add" && !$chkMe) {
        $check_user = db_stmt("SELECT id FROM ".$db['users']." WHERE `user`= ?", array('s', up($_POST['user'])),true);
        $check_nick = db_stmt("SELECT id FROM ".$db['users']." WHERE `nick`= ?", array('s', up($_POST['nick'])),true);
        $check_email = db_stmt("SELECT id FROM ".$db['users']." WHERE `email`= ?", array('s', up($_POST['email'])),true);

        $_POST['user'] = trim($_POST['user']);
        $_POST['nick'] = trim($_POST['nick']);

        if(empty($_POST['user']) || empty($_POST['nick']) || empty($_POST['email']) || ($_POST['pwd'] != $_POST['pwd2'])
            || ($check_regcode && ($_POST['confirm'] != $_SESSION['sec_reg'] || $_SESSION['sec_reg'] == NULL))
            || $check_user || $check_nick || $check_email) {
