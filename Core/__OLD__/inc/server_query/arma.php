...
$server_name = 'Armed Assault';
$server_name_short = 'ArmA';
$server_link = 'arma://{IP}:{S_PORT}';

##############################################################################################################################

function server_query_arma($ip, $port, $q_port, $request)
{
    ...
    if ($request == "players") {
        $challenge = "\xFE\xFD\x00\xAA\xAA\xAA\xAA\x00\xFF\x00";
    }
    ...
    if ($request == "players") {
        ...
        $buffer[0] = str_replace("", "name", $buffer[0]);
        ...
        do {
            foreach ($field_list as $field) {
                $player[$player_key][empty($field) ? 'name' : $field] = $item[$item_position];
