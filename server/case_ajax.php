...
function server_show($sID = 0, $showID = 0) {
    global $dir,$picformat,$db,$cache,$config_cache;
    ...
    if(!$static_server) {
            ...
            $newgq = new \GameQ\GameQ();
            $newgq->addServer(array('id' => 'gs' ,'type' => $get['game'], 'host' => $get['ip'].':'.$get['port'], 'query_port' => empty($get['qport']) ? false : $get['qport']));
            $newgq->setOption('timeout',6);
            $server = $newgq->process();
            $server = $server['gs'];
    } else
        $server = false;
    ...
    if(!empty($server) && $server && $server['gq_online']) {
        $admin_msg = ''; $playerstats = ''; $custom_teams = false;
        $image_status = '../inc/images/online.png'; //Server Status
        $image_secure = ''; $icon_mod = '';

        // Base Dir
        $icon_basic = $server['gq_protocol'].'/'.$server['gq_type'];
        $icon_mod = $server['gq_protocol'].'/'.$server['gq_mod'];
        GameQ::mkdir_img('gameicons/'.$server['gq_protocol']);

        $image_secure = ($server['game_secure']['enable'] ?
            '<img src="../inc/images/'.$server['game_secure']['pic'].'.png" alt="" title="'.$server['game_secure']['name'].'" class="icon" />'
            : '');
