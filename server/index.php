    ...
    default:
        if(fsockopen_support())
        {
            ...
            while($get = _fetch($qry))
            {
                ...
                {
                    $CachedString = $InstanceCache->getItem('gameserver_'.intval($get['id']).'_'.$language);
                    if(is_null($CachedString->get())) {
                        ...
                        if($server) {
                            $CachedString->set(serialize($server))->expiresAfter(config('cache_server'));
                            $cache->save($CachedString);
                        }
                    } else {
                        $server = unserialize($CachedString->get());
                    }
